...
class KeluarBagiHasilController extends Controller
{
	public function dataBagiHasil(){
		...
		{	
			...
			$id = '10';
			$AKKLA = Adm_jurnal::where('id_kat_jurnal',$id)->get();

			return datatables()->of($AKKLA)
		...
		return view('admin.admin-keuangan.keluar-bagi-hasil',compact('tran','kategori','satu','dua'));
	}
	...
	{	
		...
		$bukti->move(public_path('Uploads/Adm_bukti/AKKLA'), $new_name);
	...
	public function update(Request $request)
	{
		...
		{	
			...
			$foto_baru->move(public_path('Uploads/Adm_bukti/AKKLA'), $image_name);
		}
		else
		{
			$rules = array(	
	...
	public function detailBagiHasil($id)
	{
		if(request()->ajax())
		{	
