...
@section('title', 'Admin | Validasi Pembiayaan')
			...
			<script>
				$(document).ready(function(){
					$('#table_id').DataTable({
						...
						ajax: '{{ route('admin.validasi-pembiayaan') }}',
					...
					$('#sample_form').on('submit', function(event){
						...
						{
							$.ajax({
								url:"{{ route('admin.tolak-pembiayaan.update') }}",
								method:"POST",
						...
						{
							$.ajax({
								url:"{{ route('admin.validasi-pembiayaan.update') }}",
								method:"POST",
								data: new FormData(this),
								contentType: false,
								cache:false,
