...
@section('title', 'Admin | Validasi Komplain')
			...
			<script>
				$(document).ready(function(){
					$('#table_id').DataTable({
						...
						ajax: '{{ route('admin.validasi-komplain') }}',
					...
					$('#sample_form').on('submit', function(event){
						...
						{
							$.ajax({
								url:"{{ route('admin.tolak-komplain') }}",
								method:"POST",
						...
						{
							$.ajax({
								url:"{{ route('admin.proses-komplain') }}",
								method:"POST",
								data: new FormData(this),
								contentType: false,
								cache:false,
