...
@section('title', 'Admin Keuangan | Laporan Neraca')
					...
					<table id="order_table" class="table table-striped table-bordered" border="0" style="width:100%">
						...
						<tfoot>
								<tr>
									...
									<th colspan="2" style="text-align:right">TOTAL JUMLAH:</th>
									<th></th>
									<th></th>
								</tr>
								<tr>
									
									<th colspan="2" style="text-align:right">SISA SALDO:</th>
									<th colspan="2" ><a id="total"></a></th>
								</tr>
						</tfoot>
					</table>
				</div>
				...
				<script>
					...
					$('#order_table').DataTable({
						"footerCallback" : function (row, data, start, end, display) {
							...
							pageTotal1 = api.column(3, {page: 'current'}).data().reduce(function (a, b) {
									return intVal(a) + intVal(b);
								}, 0);

								var numFormat1 = $.fn.dataTable.render.number( '\,', '.', 0, 'Rp ' ).display;
								$( api.column( 3 ).footer() ).html( numFormat1( pageTotal1 ) );
							...
							pageTotal2 = api.column(2, {page: 'current'}).data().reduce(function (a, b) {
								...
								$( api.column( 2 ).footer() ).html( numFormat2( pageTotal2 ) );

							var total = pageTotal1-pageTotal2;
						...
						"paging":   false,
							...
							serverSide: true,
							ajax: {
								url:'{{ route("adminkeuangan.neraca.index") }}',
								data:{from_date:from_date, to_date:to_date}
							},
							columns: [
								{data: 'akun_debit', name:'akun_debit'},
